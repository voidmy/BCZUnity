...
namespace RVO
{
    ...
    {
        //切片  
        public static int AgentSliceNum = 3;
        /**
        ...
        private class Worker
        {
            ...
            internal void stepIndex(int stepIndex,int agentSliceNum)
            {
                var len= end_/agentSliceNum;
                len = Math.Max(len, 1);
                 int start = stepIndex * len;
                 int end = (stepIndex+1) * len;
                 end = Math.Min(end_, end);
            ...
            internal void updateStepIndex(int stepIndex,int agentSliceNum)
            {
                var len= end_/agentSliceNum;
                len = Math.Max(len, 1);
                int start = stepIndex * len;
         ...
         * agent.</param>
         */
        public int addAgent(LVector2 position)
        {
         ...
         * this agent.</param>
         */
        public int addAgent(LVector2 position, LFloat neighborDist, int maxNeighbors, LFloat timeHorizon, LFloat timeHorizonObst, LFloat radius, LFloat maxSpeed, LVector2 velocity)
        {
            Agent agent = new Agent();
            agent.id_ = s_totalID;
            s_totalID++;
        ...
        public LFloat doStep()
        {
            ...
            UnityEngine.Profiling.Profiler.BeginSample("kdTree_.buildAgentTree");
            kdTree_.buildAgentTree();
            UnityEngine.Profiling.Profiler.EndSample();
            
            UnityEngine.Profiling.Profiler.BeginSample("kdTree_.workers_");
         
            // for (int block = 0; block < workers_.Length; ++block)
            // {
            //     //if (block >= IndexStep * len && block < (IndexStep + 1) * len)
            //     {
            //         workers_[block].stepIndex(IndexStep,AgentSliceNum);
            //         workers_[block].updateStepIndex(IndexStep,AgentSliceNum);
            //     }
            //     
            ...
            // }
            workers_[0].stepIndex(IndexStep,AgentSliceNum);
            //workers_[0].updateStepIndex(IndexStep,AgentSliceNum);
            workers_[0].update(null);
            IndexStep++;
            if (IndexStep >= AgentSliceNum)
            {
                IndexStep = 0;
            }
            UnityEngine.Profiling.Profiler.EndSample();
            ...
            // UnityEngine.Profiling.Profiler.BeginSample("kdTree_.workers_2222");
            // for (int block = 0; block < workers_.Length; ++block)
            // {
            //     //doneEvents_[block].Reset();
         ...
         */
        public LVector2 getAgentPosition(int agentNo)
        {
            return agents_[agentNo2indexDict_[agentNo]].position_;
        }
        
        public void SetV3Pos(int agentNo,ref Vector3 pos)
        {
            var tmp = agents_[agentNo2indexDict_[agentNo]].position_;
           pos.x = tmp.x();
           pos.z = tmp.y();
        }

        public bool getHasAgent(int agentNo)
        {
         ...
         */
        public LVector2 getAgentPrefVelocity(int agentNo)
        {
         ...
         */
        public LVector2 getAgentVelocity(int agentNo)
        {
         ...
         * retrieved.</param>
         */
        public LVector2 getObstacleVertex(int vertexNo)
        {
         ...
         */
        public void setAgentDefaults(LFloat neighborDist, int maxNeighbors, LFloat timeHorizon, LFloat timeHorizonObst, LFloat radius, LFloat maxSpeed, LVector2 velocity)
        {
            if (defaultAgent_ == null)
            {
         ...
         * position.</param>
         */
        public void setAgentPosition(int agentNo, LVector2 position)
        {
         ...
         */
        public void setAgentPrefVelocity(int agentNo, LVector2 prefVelocity)
        {
         ...
         * velocity.</param>
         */
        public void setAgentVelocity(int agentNo, LVector2 velocity)
        {
