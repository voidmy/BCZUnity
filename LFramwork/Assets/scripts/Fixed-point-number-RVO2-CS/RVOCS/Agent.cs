...
namespace RVO
{
    ...
    {
        internal List<KeyValuePair<LFloat, Agent>> agentNeighbors_ = new List<KeyValuePair<LFloat, Agent>>();
        internal List<KeyValuePair<LFloat, Obstacle>> obstacleNeighbors_ = new List<KeyValuePair<LFloat, Obstacle>>();
        internal List<Line> orcaLines_ = new List<Line>();
        ...
        private bool listCapacityInitialized_ = false;

        private void EnsureListCapacity()
        {

            if (listCapacityInitialized_)
            {
                return;
            }

            int agentNeighborCap = maxNeighbors_ > 0 ? maxNeighbors_ : 4;
            if (agentNeighbors_.Capacity < agentNeighborCap)
            {
                agentNeighbors_.Capacity = agentNeighborCap;
            }

            int obstacleNeighborCap = 8;
            if (obstacleNeighbors_.Capacity < obstacleNeighborCap)
            {
                obstacleNeighbors_.Capacity = obstacleNeighborCap;
            }

            int lineCap = agentNeighborCap * 3 + 8;
            if (orcaLines_.Capacity < lineCap)
            {
                orcaLines_.Capacity = lineCap;
            }

            if (projLines.Capacity < lineCap)
            {
                projLines.Capacity = lineCap;
            }

            listCapacityInitialized_ = true;
        }
