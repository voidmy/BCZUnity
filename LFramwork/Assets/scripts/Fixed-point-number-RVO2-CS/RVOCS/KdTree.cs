...
namespace RVO
{
    ...
    internal class KdTree
    {
        ...
        private struct LFloatPair
        {
             ...
             */
            internal LFloatPair(LFloat a, LFloat b)
            {
                a_ = a;
                b_ = b;
            }
             ...
             */
            public static bool operator <(LFloatPair pair1, LFloatPair pair2)
            {
        ...
        {
            LFloat rangeSq = LFloat.MaxValue;
            int agentNo = -1;
        ...
        private void buildAgentTreeRecursive(int begin, int end, int node)
        {
            ...
            if (end - begin > MAX_LEAF_SIZE)
            {
                ...
                LFloat splitValue =(isVertical ? agentTree_[node].maxX_ + agentTree_[node].minX_ : agentTree_[node].maxY_ + agentTree_[node].minY_);
                splitValue /= 2;
                int left = begin;
                int right = end;
        ...
        private ObstacleTreeNode buildObstacleTreeRecursive(IList<Obstacle> obstacles)
        {
            ...
            for (int i = 0; i < obstacles.Count; ++i)
            {
                ...
                for (int j = 0; j < obstacles.Count; ++j)
                {
                    ...
                    if (new LFloatPair(Math.Max(leftSize, rightSize), Math.Min(leftSize, rightSize)) >= new LFloatPair(Math.Max(minLeft, minRight), Math.Min(minLeft, minRight)))
                    {
                        break;
                    }
                }
        ...
        private void queryAgentTreeRecursive(LVector2 position, ref LFloat rangeSq, ref int agentNo, int node)
        {
            ...
            {
                LFloat distSqLeft = RVOMath.sqr(Math.Max(0, agentTree_[agentTree_[node].left_].minX_ - position.x_)) + RVOMath.sqr(Math.Max(0, position.x_ - agentTree_[agentTree_[node].left_].maxX_)) + RVOMath.sqr(Math.Max(0, agentTree_[agentTree_[node].left_].minY_ - position.y_)) + RVOMath.sqr(Math.Max(0, position.y_ - agentTree_[agentTree_[node].left_].maxY_));
                LFloat distSqRight = RVOMath.sqr(Math.Max(0, agentTree_[agentTree_[node].right_].minX_ - position.x_)) + RVOMath.sqr(Math.Max(0, position.x_ - agentTree_[agentTree_[node].right_].maxX_)) + RVOMath.sqr(Math.Max(0, agentTree_[agentTree_[node].right_].minY_ - position.y_)) + RVOMath.sqr(Math.Max(0, position.y_ - agentTree_[agentTree_[node].right_].maxY_));
        ...
        private void queryAgentTreeRecursive(Agent agent, ref LFloat rangeSq, int node)
        {
            ...
            {
                LFloat distSqLeft = RVOMath.sqr(Math.Max(0, agentTree_[agentTree_[node].left_].minX_ - agent.position_.x_)) + RVOMath.sqr(Math.Max(0, agent.position_.x_ - agentTree_[agentTree_[node].left_].maxX_)) + RVOMath.sqr(Math.Max(0, agentTree_[agentTree_[node].left_].minY_ - agent.position_.y_)) + RVOMath.sqr(Math.Max(0, agent.position_.y_ - agentTree_[agentTree_[node].left_].maxY_));
                LFloat distSqRight = RVOMath.sqr(Math.Max(0, agentTree_[agentTree_[node].right_].minX_ - agent.position_.x_)) + RVOMath.sqr(Math.Max(0, agent.position_.x_ - agentTree_[agentTree_[node].right_].maxX_)) + RVOMath.sqr(Math.Max(0, agentTree_[agentTree_[node].right_].minY_ - agent.position_.y_)) + RVOMath.sqr(Math.Max(0, agent.position_.y_ - agentTree_[agentTree_[node].right_].maxY_));
         ...
         * <param name="node">The current obstacle k-D node.</param>
         */
        private bool queryVisibilityRecursive(LVector2 q1, LVector2 q2, LFloat radius, ObstacleTreeNode node)
        {
            if (node == null)
            {
                return true;
            ...
            return point1LeftOfQ * point2LeftOfQ >= 0 && RVOMath.sqr(point1LeftOfQ) * invLengthQ > RVOMath.sqr(radius) && RVOMath.sqr(point2LeftOfQ) * invLengthQ > RVOMath.sqr(radius) && queryVisibilityRecursive(q1, q2, radius, node.left_) && queryVisibilityRecursive(q1, q2, radius, node.right_);
        }
    }
}
